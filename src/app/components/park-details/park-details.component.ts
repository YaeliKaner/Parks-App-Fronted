...
ngOnInit(): void {
      ...
      next: (features) => {
        ...
        this._parksService.getParkById(parkId).subscribe({
          next: (parkRes) => {
            ...
            this._parkDesignService.getLatestDesignsForPark(parkId).subscribe({
              next: (designs) => {
                ...
                this.features.forEach((feature) => {
                  ...
                  this.featureForms[feature.id] = this.fb.group({
                    ratingsBreakdown: [existing?.ratingsBreakdown || 'none'],
  ...
  return Object.values(this.featureForms).some(form => form.dirty || form.touched);
}
